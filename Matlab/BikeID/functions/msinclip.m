...
%
%       [cx,crinfo]=MSINCLIP(Fdat,runmod)
%
...
%           'msinclip oldhelp' to see this. The easier-to-use new form is
%           recommended, and will be supported in the future.
%
%       See also: DIBS, MSINPREP, CRESTMIN.

%Old fdident help
%MSINCLIP Multisine design with mimimum crest factor, by time domain clipping.
%
%       [cx,crx,crxmax,cry,crymax]=MSINCLIP(fv,ampv,tf,gmod,itno,ovs,N,cl0,sr,pld)
%
%       Algorithm: time domain <--> frequency domain swapping, clipping in time
%       domain.
%
%       Output arguments:
%       cx = complex amplitudes of the multisine (coefficients of the complex
%           Fourier series: the absolute values are equal to halves of
%           the real amplitudes). For the ZOHc case (see below), precompensation
%           with the inverse of the ZOH transfer function is applied, and the
%           calculated coefficients are those of the ZOH-generated excitation signal.
%           Therefore, proper generation of the time domain signal can be
%           done by applying the dev='DAC' option in msinprep.
...
%
%       Input arguments:
%       fv = vector of frequencies where the nonzero amplitudes are given:
%           the elements must be integer multiples of a df value, minimum
%           number of sines: 2.  fv must be strictly monotonously increasing.
%       ampv = absolute values of the desired nonzero complex amplitudes
%           (coefficients of the complex Fourier series at the corresponding
%           frequencies: halves of the real coefficients).
%           If any element of ampv is complex, the phases of ampv will be used
%           as starting values, otherwise the Schroeder multisine is used.
%           For elements given with value NaN, auxiliary sine amplitudes are
%           returned in cx ("snowing"): these are not included into the
%           calculation of the effective value of the useful excitation
%           signal, only help to decrease the peak value.
%           Default for ampv: ones(length(fv),1)
%           Initial values for the snowing case can be given by a special
%           coding, by multiplying the corresponding frequency values by (-1).
%       tf = (optional) complex transfer function values at the given
%           frequencies. If tf is given, input-output optimization will be
%           performed.
%       gmod = (optional) if this is given with the value 'nograph',
...
%           The value of gmod decides how the crest factor is minimized.
%           Crest factor calculation:
%              By default, a band-limited design is done.
...
%              If gmod contains the string 'ZOHc', we assume that a ZOH signal
...
%              If gmod contains the string 'ZOHd', we deal with a discrete signal.
...
%       ovs = (optional) minimal oversampling factor: the sampling frequency
...
%           effectively
%       cl0 = (optional) initial clipping level, 0<cl0<1
%       sr = (optional) allowed maximum of the slew rate (maximum steepness of
%           the waveform). After each clipping, the routine limits the
...
%       pld = minimum time between plots
%
%       Usage: [cx,crx,crxmax,cry,crymax]=...
%                      msinclip(fv,ampv,tf,gmod,itno,ovs,N,cl0,sr,pld);
%       Examples:
...
%
%       See also: DIBS, MSINPREP.

%       Algorithm: E. van den Ouderaa, J. Schoukens and J. Renneboog, "Peak
%         Factor Minimization, Using Time-Frequency Domain Swapping Algorithm,"
%         IEEE Trans. Instrum. Meas., 1988, 37/1, pp. 144-147.
